...
$msg ="";
	 ...
	 $to=$_SESSION['too'];
	 if($date == '2017-01-03'){
		$SQL = mysqli_query($con,"SELECT * FROM `1tista_c3` WHERE `booked`= 0 && `day`= '".$date."' && `from` = '".$from."' && `to` = '".$to."'");
	    }
	   else if($date =='2017-01-04'){
		$SQL = mysqli_query($con,"SELECT * FROM `2tista_c3` WHERE `booked`= 0 && `day`= '".$date."' && `from` = '".$from."' && `to` = '".$to."'");
	    }
    ...
    if($date == '2017-01-03'){
		mysqli_query($con,"UPDATE `1tista_c3` SET `booked`=1,`user_name`='$name' WHERE `s_id`='$seatNo'");
	}
	else if($date == '2017-01-04'){
		mysqli_query($con,"UPDATE `2tista_c3` SET `booked`='2017-01-03',`user_name`='$name' WHERE `s_id`='$seatNo'");
	    }
} 
?>
...
<body>
	<?php 
	 ...
	 if($date == '2017-01-03'){
		$pr = mysqli_fetch_array(mysqli_query($con,"SELECT price FROM 1tista_c3 WHERE s_id='1'"));
    
	 }
	else if($date == '2017-01-04'){
		$pr = mysqli_fetch_array(mysqli_query($con,"SELECT price FROM 2tista_c3 WHERE s_id='1'"));
	    }
	...
	<div class="container">
	    <div class="infoform">
		<form id="booking" action="book_car3.php" method='POST' class='form'>
	    <fieldset>
		<label>Class:</label>
...
$count = "";
		...
		$list ="";
		if($date == '2017-01-03'){
		$TSQL = mysqli_query($con,"SELECT * FROM `1tista_c3` WHERE `row` =$index && `day`= '$date' && `from` = '$from' && `to` = '$to'");
	    
		}
	   else if($date == '2017-01-04'){
		$TSQL = mysqli_query($con,"SELECT * FROM `2tista_c3` WHERE `row` =$index && `day`= '$date' && `from` = '$from' && `to` = '$to'");
